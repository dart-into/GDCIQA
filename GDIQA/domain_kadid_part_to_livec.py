...
class cla_linear(nn.Module):
    def __init__(self, num_classes, keep_probability, inputsize):

        super(cla_linear, self).__init__()
        self.fc1 = nn.Linear(inputsize, 1024)
        self.bn1 = nn.BatchNorm1d(1024)
        self.drop_prob = (1 - keep_probability)
        self.relu1 = nn.PReLU()
        ...
        self.bn4 = nn.BatchNorm1d(25)              #add norm
        self.sig = nn.Sigmoid()
...
class Net(nn.Module):
    def __init__(self , net1, linear2, linear3):
        super(Net, self).__init__()
        self.feature_extractor = net1
        #self.quality_analysis = linear1
        self.domain_classifier = linear2
        self.score_analysis = linear3

    def forward(self, x1, g_judge, en_f):

        features, x_feature = self.feature_extractor(x1)
        
        #domain_output = self.domain_classifier(en_f)
        
        if g_judge==1:
            en_score = self.score_analysis(en_f)
            domain_output = self.domain_classifier(en_f)
        else:
            en_score = self.score_analysis(x_feature)
            domain_output = self.domain_classifier(x_feature)
        
        #quality_output = self.quality_analysis(features)
        
        return features, domain_output, x_feature, en_score
...
def computeSpearman(dataloader_valid1, dataloader_valid2, model):
    ...
    with torch.no_grad():
        ...
        for data1, data2 in zip(dataloader_valid1, dataloader_valid2):
            ...
            label_output1, domain_output1, x_feature, en_score = model(inputs1, 0, inputs1)
            label_output2, domain_output2, x_feature, en_score = model(inputs2, 0, inputs2)
...
def dis_cla():
    linear_cla = cla_linear(1, 0.5, 1000)
    net_ex = models.resnet50(pretrained=True)
    dis_cal_model = Net1(net1=net_ex, linear=linear_cla)
    dis_cal_model.load_state_dict(torch.load('model_IQA/KADID10K_train_dis.pt'))
    return dis_cal_model

def compute_distance(features1, features2):
    
    adj_matrix = torch.zeros((1872, 1872))
    dist_all_features = torch.cdist(features1, features2, p=2)
    
    adj_matrix[:944, 944:] = dist_all_features
    adj_matrix[944:, :944] = dist_all_features.T
    
    return adj_matrix

def get_adj_matrix(dataloader_train1, dataloader_train2, model):
    all_features1 = []
    all_features2 = []
    ...
    with torch.no_grad():
        for data1 in dataloader_train1:
            ...
            all_features1.append(outputs1)

        for data2 in dataloader_train2:
            inputs2 = data2['image']
            inputs2 = Variable(inputs2.float().cuda())
            outputs2 = model(inputs2)  
            all_features2.append(outputs2)
    ...
    distance_matrix = compute_distance(all_features1, all_features2)

    return distance_matrix, all_features1, all_features2
...
def finetune_model():
    ...
    for i in range(10):
        ...
        l_net3 = BaselineModel1(1, 0.5, 1000)
        ...
        model = Net(net1 = model1, linear2 = l_net2, linear3 = l_net3)
        #model.load_state_dict(torch.load('model_IQA/KADID10K_train_IQA.pt'))
        ...
        for epoch in range(epochs):
            ...
            dis_cal_model = dis_cla()
            
            adj_matrix, _, _ = get_adj_matrix(dataloader_train1, dataloader_train2, dis_cal_model)
            ...
            if epoch == 0:
                ...
                with torch.no_grad():
                    for data1 in dataloader_train1:
                        inputs1 = data1['image']
                        inputs1 = Variable(inputs1.float().cuda())
                        label_output1, _, x_feature1, en_score1 = model(inputs1, 0, inputs1)  
                        pre_features1.append(x_feature1)

                    for data2 in dataloader_train2:
                        inputs2 = data2['image']
                        inputs2 = Variable(inputs2.float().cuda())
                        label_output2, _, x_feature2, en_score2 = model(inputs2, 0, inputs2)
                        pre_features2.append(x_feature2)

                all_pre1 = torch.cat(pre_features1, dim=0)
                all_pre2 = torch.cat(pre_features2, dim=0)
                all_pre = torch.cat((all_pre1, all_pre2), dim=0)
                
                model_g = GCN(in_channels=1872, hidden_channels=1872, out_channels=1872)
                model_g.cuda().train()
                
                x_en = model_g(all_pre, adj_matrix)
                
                params_to_update2 = list(model_g.parameters())
        
                optimizer2 = optim.Adam(params_to_update2, lr=2e-6, weight_decay=0)
            ...
            for data1, data2 in zip(dataloader_train1, dataloader_train2):
                ...
                label_output, domain_output, _, en_score = model(inputs1, 1, en_f)
                ...
                #loss1 = label_loss
                loss_contra = criterion1(label_output, en_score)
                
                #new_loss = domain_loss.clone().detach()
                
                loss_t = label_loss - 0.4 * loss_contra
...
def load_data(mod = 'train1'):
    ...
    transformed_dataset_train1 = ImageRatingsDataset(csv_file=train_path1,
                                                    root_dir='/home/user/data/Kadid/kadid10k/kadid10k/Images/',
